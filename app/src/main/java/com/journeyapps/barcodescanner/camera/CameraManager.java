...
public final class CameraManager implements Camera.AutoFocusCallback {
    ...
    private boolean takePictureOnAutoFocus;

    private static final class CameraCallback implements Camera.PreviewCallback, Camera.PictureCallback, Camera.ShutterCallback {
        ...
        public void onPreviewFrame(byte[] data, Camera camera) {
            ...
            if (cameraResolution != null && callback != null) {
                ...
                SourceData source = new SourceData(data, cameraResolution.width, cameraResolution.height, format, rotationDegrees);
        ...
        public void onPictureTaken(final byte[] data, final Camera camera) {
            Size cameraResolution = resolution;
            if (cameraResolution != null && pictureCallback != null) {
                int format = camera.getParameters().getPreviewFormat();
                SourceData source = new SourceData(data, cameraResolution.width, cameraResolution.height, format, rotationDegrees);
                pictureCallback.onPictureTaken(source);
            }
        }
    ...
    public void startPreview() {
        ...
        if (theCamera != null && !previewing) {
            ...
            previewing = true;
            autoFocusManager = new AutoFocusManager(camera, settings, this);
    ...
    public void stopPreview() {
        ...
        if (camera != null && previewing) {
            camera.stopPreview();
            cameraCallback.setCallback(null);
            previewing = false;
        }
    }
    ...
    private void setParameters() {
        ...
        cameraCallback.setResolution(previewSize);
        cameraCallback.setRotationDegrees(getCameraRotation());
    }

    void setPictureSize() {
        try {
            Camera.Parameters parameters = camera.getParameters();
            List<Camera.Size> pitcureSizes = parameters.getSupportedPictureSizes();

            Collections.sort(pitcureSizes, new Comparator<Camera.Size>() {
                @Override
            ...
            for (Camera.Size size : pitcureSizes) {
                if (size.width * size.height <= 2048 * 1536) {
                    Log.w(TAG, "PICTURE WIDTH: " + size.width + ", HEIGHT: " + size.height);
    ...
    public void requestPreviewFrame(PreviewCallback callback) {
        Camera theCamera = camera;
        if (theCamera != null && previewing) {
            cameraCallback.setCallback(callback);
            theCamera.setOneShotPreviewCallback(cameraCallback);
        }
    }
    ...
    public void takePicture(PictureCallback callback) {
        ...
        if (theCamera != null && previewing) {
            ...
            cameraCallback.setPictureCallback(callback);
//            if (autoFocusManager.isFocusing()) {
            if (autoFocusManager.isUseAutoFocus()) {
                takePictureOnAutoFocus = true;
            } else {
                autoFocusManager.stop();
                theCamera.takePicture(cameraCallback, null, cameraCallback);
            }
        }
    }
    ...
    public void onAutoFocus(boolean b, Camera camera) {
        if (takePictureOnAutoFocus) {
            takePictureOnAutoFocus = false;
            autoFocusManager.stop();
            getCamera().takePicture(cameraCallback, null, cameraCallback);
        } else {
            autoFocusManager.queueNext();
        }
    }
}
